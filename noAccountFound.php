    ...
    <div class="col-md-3">
        ...
        <div class="col-md-9 content">
            <div class="content-body">
                <h2 style="color:#55B1F0;margin-left:200px;">" No Account Found List"</h2>
                <hr>

                <form action="accountInfoApi.php" method="post">
                    <div class="alert alert-danger">
                        <input type="text" name="searchText" placeholder="Notice Reference No">
                        <input type="hidden" name="noaccount" value="noaccount" >
            ...
            <div class="content-body" id="summaryInfo" style="display:none">
                ...
                <table id="example" class="table display table-stripped table-bordered" style="width:100%">
                    ...
                    <tbody>
                        ...
                        if($_REQUEST["r"]=="a")
                        {
                            $sql = "SELECT fsib_description, fsib_branch_code, branch_name FROM `fsb_notice_rep` left outer join fsib_branch on fsib_branch.branch_code = fsb_notice_rep.fsib_branch_code where upper(ltrim(rtrim(accountInfo))) = 'NOT FOUND'";                            
                            ///$val = $_REQUEST["r"];
                        }
                        else if($_REQUEST["r"]!=null)
                        {
                            $sql = "SELECT fsib_description, fsib_branch_code, branch_name FROM `fsb_notice_rep` left outer join fsib_branch on fsib_branch.branch_code = fsb_notice_rep.fsib_branch_code where upper(ltrim(rtrim(accountInfo))) = '".strtoupper('Not Found')."' and notice_refno = '".$_REQUEST["r"]."'";                           
                            //$eval = $_REQUEST["r"];
                        }
                        else{
                            //echo "oops";
                ...
                <a href = "report_pdf.php?token=nacc&&val=<?php echo $_REQUEST["r"];?>"><input type="submit" value="Print" name="report_submit" style="cursor:pointer; padding:7px; font-weight:bold;" /></a>
            </div>
        </div>
    </div>
    <script>
